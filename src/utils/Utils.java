	...
	public static void clearPreviousData() throws IOException {
		...
		if (Configurations.clearInputData) {
			...
			if (runConfiguration == 1) {
				...
				FileUtils.deleteDirectory(new File(FilePaths.paralelTrainingDataPath));
			}
		...
		new File(FilePaths.scaling_range_file_name).delete();
		...
		if(Configurations.clearInputData) {
			...
			if (runConfiguration == 1) {
				...
				new File(FilePaths.paralelTrainingDataPath).mkdirs();
			}
