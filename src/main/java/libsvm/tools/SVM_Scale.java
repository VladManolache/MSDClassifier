	...
	public void run(String []argv) throws IOException
	{
		...
		for(i=0;i<argv.length;i++)
		{
			...
			switch(argv[i-1].charAt(1))
			{
				...
				case 'i': input_filename = argv[i];  break;
		...
		try {
			fp = new BufferedReader(new FileReader(input_filename));
		} catch (Exception e) {
			System.err.println("can't open file " + input_filename);
			System.exit(1);
		}
		...
		try {
			outputFile = new BufferedWriter(new FileWriter(output_filename));
		} catch(IOException e) {
		...
		while(readline(fp) != null)
		{
			...
			StringTokenizer st = new StringTokenizer(line," \t\n\r\f:");
			target = Double.parseDouble(st.nextToken());
			outputFile.write(output_target(target));
